...
class categoryService {
    ...
    insertRecipeCuisine(recipe_id, cuisine_name) {
        return this.knex("recipes_cuisines")
            ...
            .then(() => {
                console.log(`(insertRecipeCuisine) Success ${recipe_id}, ${cuisine_name}`);
            })
            .catch((error) => {
                console.log(`(insertRecipeCuisine) Error ${recipe_id}, ${cuisine_name}, ${error}`);
            });
    }
    ...
    checkRecipeCuisineExist(recipe_id, cuisine_name) {
        return this.knex("recipes_cuisines")
            ...
            .then((rows) => {
                console.log("(checkRecipeCuisineExist) checkRecipeCuisineExist: ", rows);
                return !!(rows.length);
            })
            .catch((error) => {
    ...
    addRecipeCuisineIfNotExist(recipe_id, cuisine_name) {
        return this.checkRecipeCuisineExist(recipe_id, cuisine_name)
            .then((isExist) => {
                if (!isExist) {
