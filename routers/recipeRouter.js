  ...
  async fetchRecipe(request, response) {
    ...
    return this.recipeService
            ...
            this.recipeService
              ...
              .then(async (recipe_id) => {
                ...
                for (let j = 0; j < ingredients_array.length; j++) {
                  ...
                  console.log(
                    `(recipeRoute)ingredient_id: ${ingredient["ingredient_id"]}`
                  );
      ...
      .then(async (apiData) => {
        ...
        apiData["recipe_id"] = await this.recipeService
          .getRecipeByApiId(apiData["id"])
          .then((rows) => {
            console.log(rows);
            return rows[0]["id"];
          });
        ...
        const recipeReview = await this.reviewService.listall(
          apiData["recipe_id"],
          1
        );
